...
const AppearanceScreen = () => {
    ...
    const { settings, setAppearance, t } = useAppSettings();
    ...
    const renderOption = (id, title, desc, icon) => {
        const isActive = theme === id;
        return (
            <AnimatedPressable key={id} style={[st.optionCard, isActive && st.optionCardActive]} onPress={() => selectTheme(id)} scaleValue={0.97}>
                <View style={[st.iconWrap, isActive && st.iconWrapActive]}>
                    <Ionicons name={icon} size={24} color={isActive ? colors.accentPrimary : colors.textTertiary} />
                </View>
        ...
        <View style={st.root}>
            ...
            <View style={st.header}>
                ...
                <View style={st.headerCenter}>
                    <Text style={st.headerTitle}>{t('settings.appearance')}</Text>
                </View>
            ...
            <ScrollView contentContainerStyle={st.content} showsVerticalScrollIndicator={false}>
                <FadeInView delay={100} distance={15}>
                    <Text style={st.sectionLabel}>{t('settings.theme')}</Text>
                    <View style={st.optionsGroup}>
                        {renderOption('system', t('settings.themeSystem'), t('settings.themeSystemDesc'), 'phone-portrait-outline')}
                        {renderOption('light', t('settings.themeLight'), t('settings.themeLightDesc'), 'sunny-outline')}
                        {renderOption('dark', t('settings.themeDark'), t('settings.themeDarkDesc'), 'moon-outline')}
                    </View>
                    <Text style={st.helperText}>{t('settings.themeHelper')}</Text>
...
const st = StyleSheet.create({
    ...
    iconWrapActive: { backgroundColor: 'rgba(99,102,241,0.1)' },
